...
object JwtHelper {
  ...
  val config =
    KeyConfig.fromTestConfig.getOrElse(throw new Exception("Couldn't read unit test config file with JWT details."))

  def generateValidJwt(accountId: String, expiresInSec: Long = 120) = {
    val claim = JwtClaim({
      raw"""{"pid": ${config.projectId},"sid": "${config.segmentId}","kid": ${config.serviceKeyId}}"""
    }).about(accountId)
  ...
  def stringToPrivateKey(pemfile: String): java.security.PrivateKey = {
    val privateKeyBytes =
      scodec.bits.ByteVector.fromBase64(pemfile).getOrElse(throw new Exception("'$pemfile' was not base64"))
